...
public class SubmitController extends Controller {
    ...
    protected boolean customValidate(Practice practice) {
        if(AttendDateUtil.checkPast(practice.getStartDate())){
            errors.put("inputError", "今日より前の日は登録できません。");
            return false;
        }
    ...
    protected Practice construct() {
        ...
        // メール送信対象かどうか
        practice.setNeedSendMail(BooleanUtil.toPrimitiveBoolean(asBoolean("needSendMail")));

        // 出欠計算対象かどうか
        practice.setNeedCalc(BooleanUtil.toPrimitiveBoolean(asBoolean("needCalc")));
